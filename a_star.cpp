...
cv::Mat convertVectorToMat(const std::vector<std::vector<int>>& vec) {  
    ...
    // 获取内部向量的大小（假设所有内部向量大小相同）  
    int rows = vec.size();  
    int cols = (vec.empty()) ? 0 : vec[0].size();  
  
    // 创建一个3通道的Mat，数据类型为8位无符号整数（CV_8UC3）  
...
void A_Star::print_map(std::vector<std::vector<int>> &map) {
    cv::Mat result = convertVectorToMat(map);
    Node* curr_node = m_end->prev;//将除了起点和终点的路径标为'*'
    while (curr_node->prev != nullptr) {
        map[curr_node->y][curr_node->x] = 0;
        curr_node = curr_node->prev;

        cv::Vec3b color = cv::Vec3b(0, 0, 255);
        result.at<cv::Vec3b>(curr_node->y,curr_node->x) = color;
    }
    cv::imwrite("/home/sunqiang/project/A_star/result.png",result);
}